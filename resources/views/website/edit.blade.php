...
<div class="container pd-x-0">
    <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
        <div>
            ...
            <h4 class="mg-b-0 tx-spacing--1">Edit site info</h4>
        </div>
    </div>
        ...
        <div class="row row-sm">
            <div class="col-lg-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('company') is-invalid @enderror" name="company" id="company" value="{{ $website->company }}" required>
                    @error('company')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('website_name') is-invalid @enderror" name="website_name" id="website_name" value="{{ $website->website_name }}" required>
                    @error('website_name')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('project_type') is-invalid @enderror" name="project_type" id="project_type" value="{{ $website->project_type }}" required>
                    @error('project_type')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <select id="module_id" class="selectpicker mg-b-5" name="module_id[]" multiple data-style="btn btn-outline-light btn-md btn-block tx-left" data-width="100%" required>
                        @forelse($modules as $module)
                            <option value="{{ $module->id }}" @if(in_array($module->id, array_column(json_decode($website->modules, true), 'id'))) selected @endif>{{ strtoupper($module->name) }}</option>
                        @empty
                        @endforelse
                    </select>
                    @error('module_id')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('contact_person') is-invalid @enderror" name="contact_person" id="contact_person" value="{{ $website->contact_person }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('contact_number') is-invalid @enderror" name="contact_number" id="contact_number" value="{{ $website->contact_number }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('url') is-invalid @enderror" name="url" id="url" value="{{ $website->url }}" required>
                    @error('url')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
